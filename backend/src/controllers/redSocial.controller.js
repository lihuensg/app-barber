...
export const crearPost = async (req, res) => {
  try {
    ...
    const nuevoPost = await Post.create({ imagen_url, descripcion, usuario_id });
...
export const darLike = async (req, res) => {
  try {
    ...
    const yaExiste = await Like.findOne({ where: { usuario_id, post_id: postId } });
    if (yaExiste) {
...
export const comentarPost = async (req, res) => {
  try {
    ...
    const nuevoComentario = await Comentario.create({ contenido, usuario_id, post_id: postId });
